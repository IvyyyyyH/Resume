...
function oneCat(catNum){
    (function loop(i) {
        if (i < postcodes.length) new Promise(resolve => {
            var url = generateUrl(categories[catNum], postcodes[i]);
            var filename = './data/'+ categories[catNum] + '.json'
            fs.open(filename,'r',function(err, fd){
                if(err){
                    //file did not exist, create file and add the contact num
                    fs.writeFile(filename, '{"table": []}', function(err) {
                        if(err) throw err;
                    });
                }
            })
            startRequest(categories[catNum], url, filename, 20);
...
function together(numofCat){
    (function loop(i){
        if(i < numofCat) new Promise(resolve=>{
            console.log('now getting category: '+categories[i]+'...')
            oneCat(i);
            setTimeout(resolve, postcodes.length*1000);
        }).then(loop.bind(null, i+1));
    })(0);
}
