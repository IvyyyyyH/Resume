...
class LightSource {
public:
	virtual void shade(Ray3D&, double);
	...
	virtual vector<Ray3D> get_shadow_rays(Ray3D& ray) = 0;
};
...
public:
	...
	vector<Ray3D> get_shadow_rays(Ray3D& ray);
...
public:
	Extendedlight(Point3D pos, Vector3D p, Vector3D q, Colour col) : _pos(pos), _p(p), _q(q), _col_ambient(col),
		_col_diffuse(col), _col_specular(col) {}
	Extendedlight(Point3D pos, Vector3D p, Vector3D q, Colour ambient, Colour diffuse, Colour specular)
